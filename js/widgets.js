    ...
    const Widgets = function() {
        ...
        function createCustomDialog(settings) {
            ...
            var dialog_settings = {
                ...
                open: function(event, ui) {
                    ...
                    const is_other_dialog_opened = num_opened_dialog > 1;
                ...
                close: function(event, ui) {
                    ...
                    const is_other_dialog_opened = num_opened_dialog > 0;
                    if (!is_other_dialog_opened) {
                        // Check if parent element is specified
                        if (typeof settings["parent"] === "undefined") {
                            const $body = $("body");
       ...
       */
        function setCustomDropdown($ui, settings) {
            ...
            for (let i = 0; i < items.length; i++) {
                const $item = $(`<a href="javascript:void(0)">${items[i]}</a>`);
       ...
       */
        function createUnsplashPhotoHTML(credit, imageUrl) {
            const html = `<figure style="display: none;"><img src="${imageUrl}"><div>${credit}</div></figure>`;
            const $html = $(html);
            $html.find("img")
                .one("load",
                    function() {
                            ...
                            $.getJSON(targetPhotoURL,
                                    function(data) {
                                        ...
                                        for (let i = 0; i < data.length; i++) {
                                            ...
                                            const credit =
                                                `Credit: <a href="${d["user"]["links"]["html"]}" target="_blank">${d[
                                                    "user"][
                                                    "name"]}</a>`;
